...
class SparseStructureLatentVisMixin:
    ...
    def visualize_sample(self, x_0: Union[torch.Tensor, dict], random_offset: bool = True):
        ...
        yaws_offset = np.random.uniform(-np.pi / 4, np.pi / 4) if random_offset else 0
        yaws = [y + yaws_offset for y in yaws]
        pitch = [np.random.uniform(-np.pi / 4, np.pi / 4) for _ in range(4)] if random_offset else np.linspace(-np.pi / 6, np.pi / 6, 4)
