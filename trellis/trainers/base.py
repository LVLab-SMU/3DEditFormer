...
class Trainer:
    ...
    def __init__(self,
        ...
        *,
        ...
        i_ddpcheck=10000,
        init_sample=True,
        init_dataset_vis=True,
        lora=False,
        no_sample_images=False,
        load_ema_model_for_inference=False,
        **kwargs
    ):
        ...
        self.init_dataset_vis = init_dataset_vis
        self.lora = lora
        self.no_sample_images = no_sample_images
        self.load_ema_model_for_inference = load_ema_model_for_inference
        ...
        if load_dir is not None and step is not None:
            ...
            self.finetune_from(os.path.expanduser(finetune_ckpt))
            # init editing weights
            self.models['denoiser']._init_editing_weights()
    ...
    def run(self):
        ...
        if not self.no_sample_images:
            if self.is_master and self.init_dataset_vis:
                print('\nStarting training...')
        ...
        while self.step < self.max_steps:
            ...
            # Sample images
            if self.step % self.i_sample == 0 and not self.no_sample_images:
                self.snapshot()
            ...
            if self.is_master:
                ...
                if self.step % self.i_log == 0:
                    ## save to log file
                    log_str = '\n'.join([
                        f'{step}: {json.dumps(str(log))}' for step, log in log
                    ])
