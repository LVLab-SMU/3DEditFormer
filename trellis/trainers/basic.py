...
class BasicTrainer(Trainer):
    ...
    def _state_dicts_to_master_params(self, master_params, state_dicts):
        ...
        """
        master_params_names = sum(
            [[(name, n, p) for n, p in model.named_parameters() if p.requires_grad] for name, model in self.models.items()]
        , [])
        # params = [state_dicts[name][param_name] for name, param_name in master_params_names]
        params = []
        for name, param_name, param in master_params_names:
            if param_name in state_dicts[name]:
                params.append(state_dicts[name][param_name])
            else:
                assert 'editing' in param_name or 'lora' in param_name
                params.append(param)
    ...
    def load(self, load_dir, step=0):
        ...
        if self.is_master:
            ...
            if self.load_ema_model_for_inference:
                print(f'Loading ema model instead of step model for inference')

        model_ckpts = {}
        for name, model in self.models.items():
            if self.load_ema_model_for_inference:
                assert len(self.ema_rate) == 1
                model_ckpt = torch.load(read_file_dist(os.path.join(load_dir, 'ckpts', f'{name}_ema{self.ema_rate[0]}_step{step:07d}.pt')), map_location=self.device, weights_only=True)
            else:
        ...
        if self.is_master:
            ...
            for i, ema_rate in enumerate(self.ema_rate):
                ema_ckpts = {}
                for name, model in self.models.items():
                    if not os.path.exists(os.path.join(load_dir, 'ckpts', f'{name}_ema{ema_rate}_step{step:07d}.pt')):
                        need_break = True
                        break
        ...
        if os.path.exists(os.path.join(load_dir, 'ckpts', f'misc_step{step:07d}.pt')):
            ...
            if self.is_master:
                print(f'\nNo misc_step{step:07d}.pt found at step {step}, skipping...', end='')
    ...
    def finetune_from(self, finetune_ckpt):
        ...
        if self.is_master:
            print('\nFinetuning from:')
            if isinstance(finetune_ckpt, str):
                print(f'  - {finetune_ckpt}')
            else:
        ...
        if isinstance(finetune_ckpt, str):
            if not os.path.exists(finetune_ckpt):
                ...
                del pipeline
            model_ckpt = load_file(finetune_ckpt)
            model_ckpt = {k: v.to(self.device) for k, v in model_ckpt.items()}
            finetune_ckpt = {'denoiser': model_ckpt}

        model_ckpts = {}
        for name, model in self.models.items():
            model_state_dict = model.state_dict()
            if name in finetune_ckpt:
                model_ckpt = torch.load(read_file_dist(finetune_ckpt[name]), map_location=self.device, weights_only=True) if not isinstance(finetune_ckpt, dict) else finetune_ckpt[name]
                if self.lora:
                    model_ckpt_lora = dict()
                for k, v in model_ckpt.items():
                    if self.lora:
                        if 'model.' + k in model_state_dict:
                            model_ckpt_lora['model.' + k] = model_ckpt[k]
                        elif 'model.' + k.replace(k.split('.')[-1], 'base_layer.' + k.split('.')[-1]) in model_state_dict: 
                            model_ckpt_lora['model.' + k.replace(k.split('.')[-1], 'base_layer.' + k.split('.')[-1])] = model_ckpt[k]
                        else:
                            raise ValueError(f'{k} not found in model_state_dict')
                    else:
                ...
                if self.lora:
                    model_ckpt = model_ckpt_lora
                model_ckpts[name] = model_ckpt
                incompatible_keys = model.load_state_dict(model_ckpt, strict=False)
                assert len(incompatible_keys.unexpected_keys) == 0 and sum([('editing' in s) or ('lora' in s) for s in incompatible_keys.missing_keys]) == len(incompatible_keys.missing_keys)
                if self.fp16_mode == 'inflat_all':
                    model.convert_to_fp16()
            else:
                if self.is_master:
