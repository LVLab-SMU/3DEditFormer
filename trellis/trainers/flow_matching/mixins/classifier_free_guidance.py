...
class EditingClassifierFreeGuidanceMixin(ClassifierFreeGuidanceMixin):

    def get_cond(self, ori_cond_img, edited_cond_img, ori_ss_latent, **kwargs):
        assert self.p_uncond == 0, "p_uncond must be 0 for editing, i.e. classifier-free guidance is not allowed"
        return {'ori_cond_img': ori_cond_img, 'edited_cond_img': edited_cond_img, 'ori_ss_latent': ori_ss_latent, **kwargs}
