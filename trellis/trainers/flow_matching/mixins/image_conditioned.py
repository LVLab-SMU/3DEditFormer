...
class ImageConditionedMixin:
    ...
    def _init_image_cond_model(self, device=None):
        """
...
class EditingImageConditionedMixin(ImageConditionedMixin):
    ...
    def get_cond(self, ori_cond_img, edited_cond_img, ori_ss_latent, **kwargs):
        ...
        Get the conditioning data.
        """
        ori_cond_img = self.encode_image(ori_cond_img)
        edited_cond_img = self.encode_image(edited_cond_img)
        kwargs['neg_cond'] = torch.zeros_like(ori_cond_img)
        cond = super(ImageConditionedMixin, self).get_cond(ori_cond_img, edited_cond_img, ori_ss_latent, **kwargs)
        return cond
    ...
    def get_inference_cond(self, ori_cond_img, edited_cond_img, ori_ss_latent, **kwargs):
        ...
        edited_cond_img = self.encode_image(edited_cond_img)
        kwargs['neg_cond'] = torch.zeros_like(ori_cond_img)
        cond = super(ImageConditionedMixin, self).get_inference_cond(ori_cond_img, edited_cond_img, ori_ss_latent, **kwargs)
        return cond

    def vis_cond(self, ori_cond_img, edited_cond_img, ori_ss_latent, **kwargs):
        """
        Visualize the conditioning data.
        """
        return {'image': {'value': edited_cond_img, 'type': 'image'}}
