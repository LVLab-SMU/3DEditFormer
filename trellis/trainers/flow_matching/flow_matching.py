...
class FlowMatchingTrainer(BasicTrainer):
    ...
    def run_snapshot(
        ...
        for i in range(0, num_samples, batch_size):
            ...
            noise = torch.randn_like(data['x_0']) if 'x_0' in data else torch.randn_like(data['edited_ss_latent'])
            sample_gt.append(data['x_0'] if 'x_0' in data else data['edited_ss_latent'])
...
# xrh added: Editing
class EditingFlowMatchingTrainer(FlowMatchingTrainer):
    ...
    def training_losses(
        ...
        **kwargs
        ...
        # get feats_3d_t and ori_ss_latents_weights
        if hasattr(self.training_models['denoiser'], 'module'):
            cond_t1, cond_t2 = self.training_models['denoiser'].module.feats_3d_t  # [0.1, 0.9]
            ori_ss_latents_weights = self.training_models['denoiser'].module.ori_ss_latents_weights
            replace_ori_ss_latents_with_noise = self.training_models['denoiser'].module.replace_ori_ss_latents_with_noise
            attention_block_type_str = self.training_models['denoiser'].module.attention_block_type_str
        else:
            cond_t1, cond_t2 = self.training_models['denoiser'].feats_3d_t  # [0.1, 0.9]
            ori_ss_latents_weights = self.training_models['denoiser'].ori_ss_latents_weights
            replace_ori_ss_latents_with_noise = self.training_models['denoiser'].replace_ori_ss_latents_with_noise
            attention_block_type_str = self.training_models['denoiser'].attention_block_type_str

        noise = torch.randn_like(edited_ss_latent)  # [B, 8, 16, 16, 16]
        if ori_ss_latents_weights != 0.0:
            noise = noise * (1 - ori_ss_latents_weights) + ori_ss_latent * ori_ss_latents_weights

        t = self.sample_t(edited_ss_latent.shape[0]).to(edited_ss_latent.device).float()  # [B]
        x_t = self.diffuse(edited_ss_latent, t, noise=noise)  # [B, 8, 16, 16, 16]
        ...
        if attention_block_type_str != 'EditingModulatedTransformerCrossBlockV0':
            ...
            with torch.no_grad():
                ...
                _, feats_3d_1 = self.training_models['denoiser'](
                    ori_ss_latent, 
                    (torch.zeros_like(t) + cond_t1) * 1000, 
                    cond['neg_cond'], 
                    get_feats_3d=True, 
                    **cond, 
                    **kwargs
                ...
                _, feats_3d_2 = self.training_models['denoiser'](
                    ori_ss_latent if not replace_ori_ss_latents_with_noise else noise, 
                    (torch.zeros_like(t) + cond_t2) * 1000, 
                    cond['edited_cond_img'], 
                    get_feats_3d=True, 
                    **cond, 
                    **kwargs
                )
        else:
            feats_3d_1, feats_3d_2 = None, None
...
class EditingFlowMatchingCFGTrainer(EditingClassifierFreeGuidanceMixin, EditingFlowMatchingTrainer):
    pass

class EditingImageConditionedFlowMatchingCFGTrainer(EditingImageConditionedMixin, EditingFlowMatchingCFGTrainer):
    pass
